...
export const createTransactionExcel = (transaction, quotas) => {
  ...
  ws.cell(2, 9).string(transaction?.percentage + "%").style(styles["apartmentInfoHead"])
  ...
  wsBlack.cell(1, 1).string(100 - transaction?.percentage + "%").style(styles["apartmentInfoHead"])
  ...
  ws.cell(3, 9).formula(`${xl.getExcelCellRef(3, 8)} * ${transaction.percentage}%`).style(styles["apartmentInfoCell"])
  ...
  const writeQuotasHeaders = (type, row, index, sheet = ws, dollar = false) => {
    sheet.cell(row - 1, 1, row - 1, !dollar ? (index ? 14 : 12) : 9, true).string(type).style(!dollar ? styles["sectionHead"] : { ...styles["sectionHead"], fill: { type: "pattern", patternType: "solid", bgColor: "#64ff33", fgColor: "#64ff33" }, font: { size: 14, bold: true, color: "#000000" } })
    ...
    if (!dollar) {
      ...
      sheet.cell(row, 6 + addRowIfIndex).string("Cuota final").style(styles["sectionInfoHead"])
      ...
      sheet.cell(row, 13 + addRowIfIndex).string("Cuota actual").style(styles["sectionInfoHead"])
    } else {
  ...
  if (!transaction?.white?.baseIndex && !transaction?.black?.baseIndex) { // FORMATO PORCENTAJES --------------------------------------------------------------------------
      ...
      if (quota.paidUSD == null || quota.paid) {
        ...
        if (i && lastMode == "pesos") {
          if (quota?.extraAdjustment > 0 && lastMode == "pesos") {
            ...
            ws.cell(lastRow + i, 5).formula(quota?.extraAdjustmentToQuota ? `${xl.getExcelCellRef(lastRow + i, 4)} * ${xl.getExcelCellRef(lastRow + i - 1, 3)} / 100` : "0").style(styles["quota"])
          ...
          ws.cell(lastRow + i, 5).formula(quota?.adjustmentToQuota ? `${xl.getExcelCellRef(lastRow + i, 4)} * ${xl.getExcelCellRef(lastRow + i - (quota.extraAdjustment > 0 ? 2 : 1), 3)} / 100` : "0").style(styles["quota"])
          ws.cell(lastRow + i, 6).string("").style(styles["quota"])
          ws.cell(lastRow + i, 7).string("").style(styles["quota"])
          ws.cell(lastRow + i, 8).string("").style(styles["quota"])
          adjustmentRows++
        ...
        if (quota.paid != null) {
          ...
          lastMode == "pesos" && ws.cell(lastRow + i, 3).formula(!i ? xl.getExcelCellRef(3, 11) + "/" + xl.getExcelCellRef(3, 12) : `${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 6)} + ${(quota.extraAdjustment > 0 ? xl.getExcelCellRef(lastRow - 2 + i, 5) : "")} + ${xl.getExcelCellRef(lastRow - 1 + i, 5)} ${quota?.doubleAdjustmentToBalance ? ` + IF(${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)} > 0, ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)} * (${(quota.extraAdjustment > 0 ? xl.getExcelCellRef(lastRow - 2 + i, 4) + "+" + xl.getExcelCellRef(lastRow - 1 + i, 4) : xl.getExcelCellRef(lastRow - 1 + i, 4))})% + ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)}, 0)` : ""}`).style(styles["quota"])
          ...
          lastMode == "pesos" && ws.cell(lastRow + i, 6).formula((lastMode == "pesos" ? (quota?.doubleAdjustmentToBalance ? `IF(${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)} >= 0, ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 5)}, ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 5)} + ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)})` : `${totalCell} + ${xl.getExcelCellRef(lastRow + i, 5)} + ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)}`) : `${xl.getExcelCellRef(lastRow + i, 3)} + ${xl.getExcelCellRef(lastRow + i, 5)} + ${xl.getExcelCellRef(lastRow + i - adjustmentRows, 14)}`)).style(styles["quota"])
          ...
          ws.cell(lastRow + i, 10).number(quota?.interest || 0).style(styles["quota"])
        ...
        ws.cell(lastRow + i, 8).number(quota?.interest || 0).style(styles["quota"])
        ...
        if (quota.paidUSD == null || quota.paid) {
          ...
          if (i && lastMode == "pesos") {
            if (quota?.extraAdjustment > 0 && lastMode == "pesos") {
              ...
              wsBlack.cell(lastRow + i, 5).formula(quota?.extraAdjustmentToQuota ? `${xl.getExcelCellRef(lastRow + i, 4)} * ${xl.getExcelCellRef(lastRow + i - 1, 3)} / 100` : "0").style(styles["quota"])
            ...
            wsBlack.cell(lastRow + i, 5).formula(quota?.adjustmentToQuota ? `${xl.getExcelCellRef(lastRow + i, 4)} * ${xl.getExcelCellRef(lastRow + i - (quota.extraAdjustment > 0 ? 2 : 1), 3)} / 100` : "0").style(styles["quota"])
            wsBlack.cell(lastRow + i, 6).string("").style(styles["quota"])
            wsBlack.cell(lastRow + i, 7).string("").style(styles["quota"])
          ...
          if (quota.paid != null) {
            ...
            lastMode == "pesos" && wsBlack.cell(lastRow + i, 3).formula(!i ? "C2" + "/" + "D2" : `${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 6)} + ${(quota.extraAdjustment > 0 ? xl.getExcelCellRef(lastRow - 2 + i, 5) : "")} + ${xl.getExcelCellRef(lastRow - 1 + i, 5)} ${quota?.doubleAdjustmentToBalance ? `+ IF(${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)} > 0, ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)} * (${(quota.extraAdjustment > 0 ? xl.getExcelCellRef(lastRow - 2 + i, 4) + "+" + xl.getExcelCellRef(lastRow - 1 + i, 4) : xl.getExcelCellRef(lastRow - 1 + i, 4))})% + ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)}, 0)` : ""}`).style(styles["quota"])
            ...
            lastMode == "pesos" && wsBlack.cell(lastRow + i, 6).formula((lastMode == "pesos" ? (quota?.doubleAdjustmentToBalance ? `IF(${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)} >= 0, ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 5)}, ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 5)} + ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)})` : `${totalCell} + ${xl.getExcelCellRef(lastRow + i, 5)} + ${xl.getExcelCellRef(lastRow - (quota.extraAdjustment > 0 ? 3 : 2) + i, 9)}`) : `${xl.getExcelCellRef(lastRow + i, 3)} + ${xl.getExcelCellRef(lastRow + i, 5)} + ${xl.getExcelCellRef(lastRow + i - adjustmentRows, 14)}`)).style(styles["quota"])
            ...
            wsBlack.cell(lastRow + i, 10).number(quota?.interest || 0).style(styles["quota"])
          ...
          wsBlack.cell(lastRow + i, 8).number(quota?.interest || 0).style(styles["quota"])
    ...
    white.forEach((quota, i) => {
      if (quota.paidUSD == null || quota.paid) {
        ...
        if (lastMode != "pesos") {
          writeQuotasHeaders("A", lastRow + i + 2, transaction?.white?.baseIndex)
          lastMode.length && ws.cell(lastRow + i + 2, 15).string("Saldo cambio de moneda").style(styles["sectionInfoHead"])
          ...
          lastMode.length && ws.cell(lastRow + i + 3, 15).number(currencyChangeDifference).style(styles["quota"])
          lastRow += 3
          lastDollarQuotas.length = 0
        }
        ...
        ws.cell(lastRow + i, 7).formula((currencyChangeDifference < 0 ? `${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)}` : (!i || lastMode != "pesos") ? quota?.doubleAdjustmentToBalance ? (`IF(${xl.getExcelCellRef(lastRow + i, 15)} > 0, ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)} + (${quota?.indexCac / white[i ? i - 1 : 0]?.indexCac * 100 - 100}) * ${xl.getExcelCellRef(lastRow + i, 15)} / 100, ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)})`) : `${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)}` : quota?.doubleAdjustmentToBalance ? (`IF(${xl.getExcelCellRef(lastRow + i - 1, 10)} >= 0, ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + (${xl.getExcelCellRef(lastRow + i - 1, 10)} + (${quota?.indexCac / white[i ? i - 1 : 0]?.indexCac * 100 - 100}) * ${xl.getExcelCellRef(lastRow + i - 1, 10)} / 100), ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i - 1, 10)})`) : `${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i - 1, 10)}`)).style(styles["quota"])
        ...
        ws.cell(lastRow + i, 11).number(quota?.interest || 0).style(styles["quota"])
        ...
        ws.cell(lastRow + i, 14).formula(`${xl.getExcelCellRef(lastRow + i, 3)} + ${xl.getExcelCellRef(lastRow + i, 6)}`).style(styles["quota"])
        lastMode = "pesos"
      } else {
        if (lastMode != "dollar") {
          writeQuotasHeaders("A", lastRow + i + 2, transaction?.white?.baseIndex, ws, true)
    ...
    if (xl.getExcelCellRef(2, 1) - xl.getExcelCellRef(2, 2) != 0) {
      ...
      black.forEach((quota, i) => {
        if (quota.paidUSD == null || quota.paid) {
          ...
          if (lastMode != "pesos") {
            writeQuotasHeaders("B", lastRow + i + 2, transaction?.black?.baseIndex, wsBlack)
            lastMode.length && wsBlack.cell(lastRow + i + 2, 15).string("Saldo cambio de moneda").style(styles["sectionInfoHead"])
            ...
            lastMode.length && wsBlack.cell(lastRow + i + 3, 15).number(currencyChangeDifference).style(styles["quota"])
            lastRow += 3
            lastDollarQuotas.length = 0
          }
          ...
          wsBlack.cell(lastRow + i, 7).formula((currencyChangeDifference < 0 ? `${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)}` : (!i || lastMode != "pesos") ? quota?.doubleAdjustmentToBalance ? (`IF(${xl.getExcelCellRef(lastRow + i, 15)} > 0, ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)} + (${quota?.indexCac / black[i ? i - 1 : 0]?.indexCac * 100 - 100}) * ${xl.getExcelCellRef(lastRow + i, 15)} / 100, ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)})`) : `${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i, 15)}` : quota?.doubleAdjustmentToBalance ? `IF(${xl.getExcelCellRef(lastRow + i - 1, 10)} >= 0, ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + (${xl.getExcelCellRef(lastRow + i - 1, 10)} + (${quota?.indexCac / black[i ? i - 1 : 0]?.indexCac * 100 - 100}) * ${xl.getExcelCellRef(lastRow + i - 1, 10)} / 100), ${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i - 1, 10)})` : `${xl.getExcelCellRef(lastRow + i, 6)} + ${totalCell} + ${xl.getExcelCellRef(lastRow + i - 1, 10)}`)).style(styles["quota"])
          ...
          wsBlack.cell(lastRow + i, 11).number(quota?.interest || 0).style(styles["quota"])
          ...
          wsBlack.cell(lastRow + i, 14).formula(`${xl.getExcelCellRef(lastRow + i, 3)} + ${xl.getExcelCellRef(lastRow + i, 6)}`).style(styles["quota"])
...
export const createFutureQuotasExcel = (transactions, lastIndexCac, indexCac, secondIndexCac) => {
  ...
  transactions.forEach((t, i) => {
    if (t.white) {
      const adjustmentIndex = indexCac / t?.white?.lastQuota?.indexCac * 100 - 100
      ...
      ws.cell(lastRow + i, 4).number(t.white?.lastQuota?.quota + 1 || 1).style(styles["subsectionInfoCell"])
      ws.cell(lastRow + i, 5).number(t?.white?.lastQuota?.paid != null ? (t.white?.updatedQuota * (t?.dolar || 1) || 0) : t?.white?.updatedQuota || 0).style(styles["subsectionInfoCell"])
      const balance = (
        t?.white?.lastQuota?.paid != null ? (
          (t.white?.updatedQuota || 0) - ((t.white?.lastQuota?.paid || 0))
        ) : t?.white?.lastQuota?.total - t?.white?.lastQuota?.paidUSD
      ) || 0
      ws.cell(lastRow + i, 6).number(
        t?.white?.lastQuota?.paid != null ? (
          t.white?.baseIndex ?
            (indexCac / t.white?.baseIndex * 100 - 100) * (t.white?.baseQuota) / 100 + (t.white?.baseQuota) + (balance <= 0 ? balance : balance + (t?.white?.lastQuota?.doubleAdjustmentToBalance ? (adjustmentIndex * balance / 100) : 0)
            ) : (
              ((balance > 0 && t?.white?.lastQuota?.doubleAdjustmentToBalance) ? (adjustment * balance / 100) : 0) + ((balance > 0 && t?.white?.lastQuota?.doubleAdjustmentToBalance) ? balance : 0) + (adjustment * t.white?.lastQuota?.total / 100) + t.white?.updatedQuota
            ) * cac / 100 + (
              (t?.white?.lastQuota?.doubleAdjustmentToBalance ? (balance > 0 ? (adjustment * balance / 100) + (balance > 0 ? balance : 0) : balance) : balance) + (adjustment * t.white?.lastQuota?.total / 100) + t.white?.updatedQuota
            )) * (t?.dolar || 1) || 0 : (
          t?.white?.baseQuota + balance
        )).style(styles["subsectionInfoCell"])
      ws.cell(lastRow + i, 7).number((t.white?.lastQuota?.interest || 0) || 0).style(styles["subsectionInfoCell"])
    ...
    if (t.black) {
      const adjustmentIndex = indexCac / t?.black?.lastQuota?.indexCac * 100 - 100
      ...
      ws.cell(lastRow + i, 13).number(t.black?.lastQuota?.quota + 1 || 1).style(styles["subsectionInfoCell"])
      ws.cell(lastRow + i, 14).number(t?.black?.lastQuota?.paid != null ? (t.black?.updatedQuota * (t?.dolar || 1) || 0) : t?.black?.updatedQuota || 0).style(styles["subsectionInfoCell"])
      const balance = (
        t?.black?.lastQuota?.paid != null ?
          ((t.black?.updatedQuota || 0) - ((t.black?.lastQuota?.paid || 0)))
          : t?.black?.lastQuota?.total - t?.black?.lastQuota?.paidUSD
      ) || 0
      ws.cell(lastRow + i, 15).number(t?.black?.lastQuota?.paid != null ?
        (t.black?.baseIndex ?
          (indexCac / t.black?.baseIndex * 100 - 100) * (t.black?.baseQuota) / 100 + (t.black?.baseQuota) + (balance <= 0 ? balance : balance + (t?.black?.lastQuota?.doubleAdjustmentToBalance ? (adjustmentIndex * balance / 100) : 0)
          ) : (
            ((balance > 0 && t?.black?.lastQuota?.doubleAdjustmentToBalance) ? (adjustment * balance / 100) : 0) + ((balance > 0 && t?.black?.lastQuota?.doubleAdjustmentToBalance) ? balance : 0) + (adjustment * t.black?.lastQuota?.total / 100) + t.black?.updatedQuota
          ) * cac / 100 + (
            (t?.black?.lastQuota?.doubleAdjustmentToBalance ? (balance > 0 ? (adjustment * balance / 100) + (balance > 0 ? balance : 0) : balance) : balance) + (adjustment * t.black?.lastQuota?.total / 100) + t.black?.updatedQuota
          )) * (t?.dolar || 1) || 0 : (
          t?.black?.baseQuota + balance)).style(styles["subsectionInfoCell"])
      ws.cell(lastRow + i, 16).number((t.black?.lastQuota?.interest || 0) || 0).style(styles["subsectionInfoCell"])
  ...
  const lastRowTotal = whiteLastRow > blackLastRow ? whiteLastRow : blackLastRow

  ws.cell(lastRowTotal, 8).formula(`SUM(${xl.getExcelCellRef(lastRow, 8)}:${xl.getExcelCellRef(lastRowTotal - 2, 8)})`).style(styles["subsectionInfoCell"])
  ws.cell(lastRowTotal, 17).formula(`SUM(${xl.getExcelCellRef(lastRow, 17)}:${xl.getExcelCellRef(lastRowTotal - 2, 17)})`).style(styles["subsectionInfoCell"])
  ws.cell(lastRowTotal, 9).formula(`SUM(${xl.getExcelCellRef(lastRow, 9)}:${xl.getExcelCellRef(lastRowTotal - 2, 9)})`).style(styles["subsectionInfoCell"])
  ws.cell(lastRowTotal, 18).formula(`SUM(${xl.getExcelCellRef(lastRow, 18)}:${xl.getExcelCellRef(lastRowTotal - 2, 18)})`).style(styles["subsectionInfoCell"])
  return wb
}