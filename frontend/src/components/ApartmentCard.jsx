...
const ApartmentCard = ({apartment, className, previousPath, edit=false}) => {
  return (
    <Link to={`/inmueble/${apartment._id}${edit ? "/edit" : ""}`} state={{from: previousPath || ""}} className={`${apartment.forSale ? "bg-fifth" : "bg-red-500"} flex flex-col items-center py-6 px-3 h-[250px] w-[200px] ${className}`}>
